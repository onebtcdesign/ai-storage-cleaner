...
生成出来的 `storage-report.html` 含有本机路径和扫描数据，只作为用户本地结果留存，不应放进 skill 源码或开源仓库。源码里保留的是 `assets/report_template.html` 通用模板。

## 版本说明

### v1.1.0

- 报告改为左侧导航 + 右侧工作台的全屏交互界面。
- 新增「应用程序」模块，展示应用体积排行，并只提供定位/访问操作。
- 大文件、重复文件和应用排行都支持服务模式下的「访问」按钮。
- 强化开源边界：生成报告含本机路径，已加入忽略规则，不应提交到仓库。

### v1.0.0

- 初始版本：只读扫描、分级分析、HTML 报告和安全移入废纸篓/回收站。
