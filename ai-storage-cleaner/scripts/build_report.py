...

{
  ...
  "scan_seconds": 42.1,
  "report": {title, computer_name},
  "cleanup_profile": {default_mode(light|medium|deep), selected_label},
  "system": {os, build, arch, user, home, computer_name, filesystem,
             disk_total, disk_used, disk_free, purgeable,
             disks:[{name,mount,total,used,free,external}]},
  "top5": [{rank, tier(green|yellow|red), size, type, name, path, note}],
  "green":  [{name, path, size_estimate, cleanup_level(light|medium|deep),
              kill_processes:[], trash_paths:[...], commands:[{label,cmd}]}],
  "yellow": [{name, path, size, cleanup_level(light|medium|deep),
